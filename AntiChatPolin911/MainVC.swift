...
class MainVC: UIViewController, PNObjectEventListener {
   ...
   // var menuTransitionManager = MenuTransitionManager()
    ...
    func client(_ client: PubNub, didReceiveMessage message: PNMessageResult) {
        ...
        var stringSticker = stringData?["stickers"] as? String ?? ""
        
        
        var newMessage = ChatMessage(username: stringName, text: stringText, time: stringTime, image: stringImg, imgSticker: stringSticker )
    ...
    func client(_ client: PubNub, didReceivePresenceEvent event: PNPresenceEventResult) {
        ...
        case "join":
            var pubChat = ChatMessage(username:"", text: "\(userName) присоединился к нам!", time: getTime(), image: "", imgSticker: "")
            
            chatMesArray.append(pubChat)
            
        case "leave":
            var pubChat = ChatMessage(username: "", text: "\(event.data.presence.uuid?.uppercased()) left the chat", time: getTime(),image: "", imgSticker: "")
            chatMesArray.append(pubChat)
...
//            chatMesArray.append(pubChat)
        ...
        default:
            var pubChat = ChatMessage(username: "", text: "\(event.data.presence.uuid?.uppercased()) - \(userName) покинул нас", time: getTime(),image: "", imgSticker: "")
            //chatMesArray.append(pubChat)
